...
//  int v_min_id=INT_MAX;  
  ...
  if(s_count == 0)
  {
    elog(ERROR, "Source edge: %d was not found as any of the input edges.", source_edge_id);
    return -1;
  }
	      
  if(t_count == 0)
  {
    elog(ERROR, "Target edge: %d was not found as any of the input edges.", target_edge_id);
    return -1;
  }
  ...
  if (SRF_IS_FIRSTCALL())
    {
      ...
      ret = compute_shortest_path_shooting_star(sql,
					PG_GETARG_INT32(1),
					PG_GETARG_INT32(2),
					PG_GETARG_BOOL(3),
					PG_GETARG_BOOL(4), 
