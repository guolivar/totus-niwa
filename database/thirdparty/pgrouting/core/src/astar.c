               ...
               path_element_t **path, int *path_count) 
{
  ...
  if(s_count == 0)
  {
    elog(ERROR, "Source vertex: %d was not found as vertex of any of the input edges.", source_vertex_id);
    return -1;
  }
        
  if(t_count == 0)
  {
    elog(ERROR, "Target vertex: %d was not found as vertex of any of the input edges.", target_vertex_id);
    return -1;
  }
  ...
  if (SRF_IS_FIRSTCALL())
    {
      ...
      ret = compute_shortest_path_astar(sql,
          PG_GETARG_INT32(1),
          PG_GETARG_INT32(2),
          PG_GETARG_BOOL(3),
          PG_GETARG_BOOL(4), 
